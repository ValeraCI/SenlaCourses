...
select (mem.firstname || ' ' || mem.surname) as member, fac.name as facility, 
	case
		when mem.memid = 0 then fac.guestcost*bk.slots
		else fac.membercost*bk.slots end
		as cost
...
where
	...
	and((mem.memid = 0 and fac.guestcost*bk.slots > 30) or (mem.memid != 0 and fac.membercost*bk.slots > 30))	
order by cost desc;
