...
with recursive temp1(recommender, member) as(
  select tb1.recommendedby, tb1.memid
  from cd.members tb1
  union
  select tb2.recommendedby, member
  from cd.members tb2 join temp1 on (temp1.recommender = tb2.memid)
  )
  
select member, recommender, firstname, surname
from temp1 t1 join cd.members mem on (t1.recommender = mem.memid)
