...
with recursive temp1(recommendedby) as(
  select tb1.recommendedby
  from cd.members tb1
  where tb1.memid = 27
  union
  select tb2.recommendedby
  from cd.members tb2 join temp1 on (temp1.recommendedby = tb2.memid)
  )
  
select t1.recommendedby as recommender, firstname, surname
from temp1 t1 join cd.members mem on (t1.recommendedby = mem.memid);