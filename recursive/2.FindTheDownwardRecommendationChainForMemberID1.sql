...
with recursive temp1(memid) as(
  select tb1.memid
  from cd.members tb1
  where tb1.recommendedby = 1
  union
  select tb2.memid
  from cd.members tb2 join temp1 on (tb2.recommendedby = temp1.memid)
  )
select t1.memid, firstname, surname
from temp1 t1 join cd.members mem on (t1.memid = mem.memid)
order by t1.memid;